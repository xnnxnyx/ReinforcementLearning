...
def get_q_learning_results(shared_data):
    results = dict()
    for case_i in shared_data:
        ...
        for alpha in [0.1, 0.5, 0.9]:
            ...
            for traj in zip(
                ...
                for transition in zip(*traj):
                    results[case_i]["compute_update"].append(
                        agent.compute_update(
                            *transition
                        )
                    )
    return results
...
if __name__ == "__main__":
    ...
    results = get_q_learning_results(shared_data)
    
    expected_results = test_data["q_learning"]
